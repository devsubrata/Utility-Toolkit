if (!document.getElementById("openYoutubePlayer")) {
    ...
    document.getElementById("channelListBtn").addEventListener("click", async () => {
        document.getElementById("channelModal").classList.toggle("hidden");
                                ...
                                <div class="bg-white rounded-xl overflow-hidden shadow hover:shadow-lg transition border border-gray-400">
                                    ...
                                    <div class="!p-4">
                                        ...
                                        <button data-id="${videoId}" data-title="${index + 1}. ${v.snippet.title}"
                                                        class="load-media-btn cursor-pointer text-sm !px-2 !py-1 bg-gray-200 hover:bg-gray-300 text-gray-700 rounded">Load Media</button>
                                    </div>
                                </div>`;
                        })
                        .join("");
    ...
    async function fetchChannelInfo(channelId, apiKey) {
        ...
        return {
            ...
            banner: ch.brandingSettings?.image?.bannerExternalUrl || "", // add banner
            subscriberCount: formatViews(ch.statistics.subscriberCount),
            videoCount: parseNumber(ch.statistics.videoCount),
    ...
    async function enrichPlaylist(pl, apiKey) {
        ...
        const totalViews = videos.reduce((s, v) => s + parseNumber(v.statistics.viewCount || "0"), 0);
    ...
    function renderPlaylists(pls, channelInfo) {
        ...
        // Title
        infoText.innerHTML = `
            <h1 class="text-xl font-bold text-gray-800">${channelInfo.title}</h1>
        ...
        pls.forEach((pl, i) => {
            ...
            miniCard.className = "cursor-pointer bg-white shadow hover:shadow-md transition rounded-lg !p-3 border border-gray-200";
            miniCard.innerHTML = `
        ...
        playListTitle.className =
            "bg-white w-1/2 !text-indigo-900 text-4xl rounded-xl shadow !p-3 !mb-6 !mx-auto overflow-hidden flex items-center justify-center";
        ...
        videosTitle.className =
            "bg-white w-2/3 !text-indigo-900 text-4xl rounded-xl shadow !p-3 !mb-6 !mx-auto overflow-hidden flex items-center justify-center";
        ...
        pls.forEach((pl, i) => {
            ...
            section.innerHTML = `
                <div class="flex items-start gap-4 !mb-4">
                    <img src="${pl.thumbnail}" alt="" class="w-32 h-20 object-cover rounded">
                    <div class="flex-1 header-right">
                        <h2 class="text-xl font-bold text-gray-800">${i + 1 + ". " + pl.title}</h2>
                        <div class="text-sm text-gray-600 !mt-0.5">
                            Playlist · ${pl.videoCount} videos · ${pl.viewCount} views
                        </div>
    ...
    generalSearchBtn.onclick = function () {
        ...
        function loadContent() {
            ...
            <div class="flex gap-4 !mb-6 justify-center items-center">
                ...
                <input id="maxVideoResults" type="number" min="10" step="10" value="50" placeholder="Max results" class="bg-white text-black !text-xl w-[75px] border-3 border-indigo-700 rounded !px-2 !py-2" />
            </div>
        ...
        generalSearchLoader.className =
            "absolute top-0 left-[40%] w-[800px] h-[900px] bg-black text-white border-2 border-[#444] shadow-[0_0_10px_rgba(0,0,0,0.5)] resize overflow-hidden flex flex-col z-[25000]";
        ...
        gsBtn.addEventListener("click", async () => {
            ...
            try {
                ...
                const searchResults = await fetchGeneralSearchResults(query, maxCount, apiKey);
                const videoIds = searchResults.map((v) => v.id.videoId);
                const statsData = await fetchVideoStats(videoIds, apiKey);

                renderGeneralSearchResults(searchResults, statsData, resultContainer);
            } catch (err) {
        ...
        function renderGeneralSearchResults(results, statsData, container) {
            container.innerHTML = "";
